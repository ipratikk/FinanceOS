...
for i in "${!GROUP_LABELS[@]}"; do
  ...
  # Generate commit message via caveman-commit (via claude -p)
  # Falls back to generic if claude unavailable
  if command -v claude &>/dev/null; then
    diff_summary=$(git diff --cached --stat 2>/dev/null | tail -5)
    diff_patch=$(git diff --cached 2>/dev/null | head -200)
    # Use caveman-commit style: terse, Conventional Commits, ≤50 char subject
    prompt="Generate a commit message (Conventional Commits format). Subject ≤50 chars. Body only if why is non-obvious. No filler. Layer: $label.\n\nStat:\n$diff_summary\n\nDiff:\n$diff_patch"
