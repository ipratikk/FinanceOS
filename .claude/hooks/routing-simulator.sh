#!/usr/bin/env bash
# Routing simulator: runs canned prompts through routing-lib, prints PASS/FAIL table.

set -e
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$HERE/routing-lib.sh"

# Test cases: "prompt|expected-agent"
declare -a CASES=(
  # ── Haiku ──
...
# Hard rules — anything that would route opus to a cheap task is a critical fail
echo
echo "── Hard-rule checks ──"
declare -a HARD_NEGATIVES=(
  "build the project"
...
[ "$HARD_OK" = "0" ] && echo "✓ no opus leaks on hard-negative prompts"

[ "$FAIL" = "0" ] && [ "$HARD_OK" = "0" ] && exit 0 || exit 1
