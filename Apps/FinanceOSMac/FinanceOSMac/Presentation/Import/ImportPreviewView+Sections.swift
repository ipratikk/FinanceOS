...
extension ImportPreviewView {
    ...
    func aggregatedSummarySection() -> some View {
        let totalTransactions = viewModel.parsedStatements.reduce(0) {
            $0 + $1.transactions.count
        }
    ...
    func aggregatedTransactionListSection() -> some View {
        ...
        let newTransactions = allTransactions.count -
            viewModel.duplicateTransactionIndices.count

        return VStack(alignment: .leading, spacing: 8) {
            transactionStatsHeader(
                newCount: newTransactions,
                duplicateCount: viewModel.duplicateTransactionIndices.count
            )
            ...
            transactionListContent(allTransactions)
        }
    }

    func transactionStatsHeader(newCount: Int, duplicateCount: Int) -> some View {
        HStack(spacing: 16) {
            VStack(alignment: .leading) {
    ...
    func transactionListContent(_ allTransactions: [ParsedTransaction]) -> some View {
    ...
    func formatAmount(_ minorUnits: Int64) -> String {
