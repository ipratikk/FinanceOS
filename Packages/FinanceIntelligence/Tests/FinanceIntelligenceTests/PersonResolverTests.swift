...
@Test func personResolver_upi_extractsNameAndHandle() {
...
@Test func personResolver_upi_seemaGoel() {
...
@Test func personResolver_neft_extractsPartyName() {
...
@Test func personResolver_neft_technologyCompanyIsNotPerson() {
    let result = resolver.resolve("NEFT CR-ICIC0001234-XYZ TECHNOLOGIES LTD-REF555")
    #expect(result == nil)
}

@Test func personResolver_neft_paymentServiceIsNotPerson() {
    let result = resolver.resolve("NEFT CR-YESB0000001-ABC PAYMENTS SERVICES-NEFTREF333")
    #expect(result == nil)
}

@Test func personResolver_neft_zomatoIsNotPerson() {
...
@Test func personResolver_imps_alwaysPersonTransfer() {
...
@Test func personResolver_nonStructured_returnsNil() {
    let result = resolver.resolve("SALARY CREDIT HDFC BANK JUNE 2026")
    #expect(result == nil)  // not a UPI/NEFT/IMPS format
}

@Test func personResolver_atmWithdrawal_returnsNil() {
    let result = resolver.resolve("ATM WITHDRAWAL HDFC BANK 12345")
    #expect(result == nil)
}

@Test func personResolver_netflixCharge_returnsNil() {
    let result = resolver.resolve("NETFLIX SUBSCRIPTION MONTHLY")
    #expect(result == nil)
}

@Test func personResolver_emptyDescription_returnsNil() {
    let result = resolver.resolve("")
    #expect(result == nil)
}

@Test func personResolver_confidence_alwaysBounded() {
    let descriptions = [
...
@Test func personEntityStore_deduplicatesBySameName() async {
...
@Test func personEntityStore_deduplicatesByUPIHandle() async {
    let store = PersonEntityStore()
    let date = Date()
...
@Test func personEntityStore_incrementsTransactionCount() async {
    let store = PersonEntityStore()
    let date = Date()
...
@Test func personEntityStore_distinctNamesCreateDistinctPersons() async {
    let store = PersonEntityStore()
    let date = Date()
