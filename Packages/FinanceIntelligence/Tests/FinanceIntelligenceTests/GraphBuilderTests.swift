...
private enum GraphBuilderTestFactory {
    static func makeStore() throws -> GraphStore {
    ...
    static func makeEnriched(txn: Transaction, merchant: String, categoryId: String) -> EnrichedTransaction {
    ...
    static func makeDiverseBatch(ledgerId: UUID) -> [EnrichedTransaction] {
...
struct GraphBuilderTopologyTests {
    @Test("Single debit creates transaction→merchant PAID_TO edge")
    func singleDebitCreatesPaidToEdge() async throws {
        let store = try GraphBuilderTestFactory.makeStore()
        let builder = GraphBuilder(store: store)

        let txn = GraphBuilderTestFactory.makeTxn(description: "UPI-BLINKIT", amount: 50000, type: .debit)
        let enriched = GraphBuilderTestFactory.makeEnriched(txn: txn, merchant: "Blinkit", categoryId: "groceries")
    ...
    func creditCreatesPaidFromEdge() async throws {
        ...
        let txn = GraphBuilderTestFactory.makeTxn(
            description: "NEFT CR-PAYPAL SALARY",
            amount: 15_000_000,
            type: .credit
        )
        let enriched = GraphBuilderTestFactory.makeEnriched(txn: txn, merchant: "PayPal", categoryId: "income")
        try await builder.build(from: [enriched])
    ...
    func repeatedMerchantIncrementsObservationCount() async throws {
        ...
        let txn1 = GraphBuilderTestFactory.makeTxn(description: "UPI-SPOTIFY", amount: 13900, type: .debit)
        let txn2 = GraphBuilderTestFactory.makeTxn(description: "UPI-SPOTIFY", amount: 13900, type: .debit)
        let enriched = [
            GraphBuilderTestFactory.makeEnriched(txn: txn1, merchant: "Spotify", categoryId: "subscriptions"),
            GraphBuilderTestFactory.makeEnriched(txn: txn2, merchant: "Spotify", categoryId: "subscriptions")
        ]
        try await builder.build(from: enriched)
    ...
    func bfsDepth2FindsCategory() async throws {
        ...
        let txn = GraphBuilderTestFactory.makeTxn(description: "UPI-DOMINOS", amount: 48100, type: .debit)
        let enriched = GraphBuilderTestFactory.makeEnriched(txn: txn, merchant: "Dominos Pizza", categoryId: "dining")
        try await builder.build(from: [enriched])
...
@Suite("GraphBuilder — 50 transaction topology")
struct GraphBuilderBulkTopologyTests {
    @Test("50 diverse transactions produce correct graph topology")
    func fiftyTransactionGraphTopology() async throws {
        let store = try GraphBuilderTestFactory.makeStore()
        let builder = GraphBuilder(store: store)
        let ledgerId = UUID()
        let transactions = GraphBuilderTestFactory.makeDiverseBatch(ledgerId: ledgerId)
...
@Suite("GraphBuilder — INTEL-016 resolvedPersonId consistency")
struct GraphBuilderResolvedPersonIdTests {
    ...
    func personResolvedCreatesPersonEdgeNotMerchant() async throws {
        ...
        let builder = GraphBuilder(store: store)
        let personId = UUID()
        let txn = GraphBuilderTestFactory.makeTxn(description: "UPI-RAHUL@HDFC", amount: 50000, type: .debit)
        let features = TransactionFeatureExtractor().extract(from: txn)
        let enriched = EnrichedTransaction(
            transaction: txn,
            merchantCandidate: MerchantCandidate(MerchantCandidateInput(
                rawDescription: txn.description,
                cleanedDescription: "Rahul",
                canonicalName: "Rahul",
                confidence: 0.9,
                source: .alias
            )),
    ...
    func nonPersonTransactionCreatesOnlyMerchant() async throws {
        let store = try GraphBuilderTestFactory.makeStore()
        let builder = GraphBuilder(store: store)
        let txn = GraphBuilderTestFactory.makeTxn(description: "UPI-ZEPTO", amount: 30000, type: .debit)
        let enriched = GraphBuilderTestFactory.makeEnriched(txn: txn, merchant: "Zepto", categoryId: "groceries")
        try await builder.build(from: [enriched])
