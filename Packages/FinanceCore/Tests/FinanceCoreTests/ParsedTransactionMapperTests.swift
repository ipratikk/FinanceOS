...
func mapperHandlesCardDebit() {
    ...
    let mapped = ParsedTransactionMapper.map(debitParsed, target: target, ledgerKind: .creditCard)
...
func mapperHandlesCardCredit() {
    ...
    let mapped = ParsedTransactionMapper.map(creditParsed, target: target, ledgerKind: .creditCard)
