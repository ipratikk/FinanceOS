...
public protocol TransactionIntelligenceWriter: Sendable {
    ...
    /// Persists the human-readable description generated by the intelligence pipeline.
    func updateEnrichedDescription(id: UUID, description: String) async throws

    /// Persists many enriched descriptions in a single transaction. Far cheaper than
    /// calling `updateEnrichedDescription` per row across a full pipeline run.
    func updateEnrichedDescriptionBatch(_ updates: [(id: UUID, description: String)]) async throws

    /// Links two reconciled counterpart transactions (e.g. bank debit ↔ card credit payment).
